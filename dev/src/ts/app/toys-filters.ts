  ...
  render(parent: HTMLElement): void {
    ...
    filterShapeCircle.onclick = (): void => {
      ...
      if (!filterShapeCircle.classList.contains('filter-shape_active')) {
        ...
        toys.renderToys(
          cards,
          'https://raw.githubusercontent.com/sergJess/stage1-tasks/christmas-task/dataJsonImg.json'
        );
      } else {
    ...
    const rangeInstanseInputs = [
      sliderRangeMinInstance,
      sliderRangeMaxInstance,
    ];
    ...
    (<API>slider.noUiSlider).on('update', function (values, handle): void {
      const toys = new ToysView();
      rangeInstanseInputs[handle].innerHTML = values[handle]
        .toString()
        .split('.')[0];
    ...
    (<API>sliderYear.noUiSlider).on('update', function (values, handle): void {
      const toys = new ToysView();
      yaerInstanseInputs[handle].innerHTML = values[handle]
        .toString()
        .split('.')[0];
    ...
    sliderRangeItem.append(
      sliderRangeMinInstance,
      filterRangeSliderInstance,
      sliderRangeMaxInstance
    );
    filterItemSecond.append(
      filterRangeTitle,
      sliderRangeTitle,
      sliderRangeItem,
      sliderCountTitle,
    ...
    sliderCountItem.append(
      sliderYearMinInstance,
      filterYearSliderInstance,
      sliderYearMaxInstance
    );
    filterItemThird.append(
      filterSortTitle,
      fliterSortSelect,
      buttonResetFilters
    );
