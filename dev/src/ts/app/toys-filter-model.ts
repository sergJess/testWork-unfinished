...
fillDefaultChosen(data:ToysData[]){
...
const data:ToysData[] = [];
for(let i=0; i<filter.length; i++){
	if(Number(filter[i].count) >= this.filterRangeSliderInstanceMin && Number(filter[i].count) <= this.filterRangeSliderInstanceMax){
		data.push(filter[i]);
	}
}
return data;
	...
	applyYearFilterInstance(filter:ToysData[]):ToysData[]{
		const data:ToysData[] = [];
		for(let i=0; i<filter.length; i++){
			if(Number(filter[i].year) >= this.filterRangeSliderYearMin && Number(filter[i].year) <= this.filterRangeSliderYearMax){
				data.push(filter[i]);
			}
		}
		return data;
	...
	setFilterRangeSliderYearMax(el:HTMLElement){
		this.filterRangeSliderYearMax = Number(el.textContent);
	}
	setFilterRangeSliderYearMin(el:HTMLElement){
		this.filterRangeSliderYearMin = Number(el.textContent);
	}
	setFilterRangeSliderInstanceMax(el:HTMLElement){
		this.filterRangeSliderInstanceMax = Number(el.textContent);
	}
	setFilterRangeSliderInstanceMin(el:HTMLElement){
